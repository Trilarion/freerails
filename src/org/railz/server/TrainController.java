 ...
 * starting and stopping of trains at train stations, and the changing of train
 * destinations.
 */
class TrainController {
    ...
    private void updateTrain(TrainModel tm, GameTime now, FreerailsPrincipal p, int trainIndex) {
	int state = tm.getState();
	/* Is the train lost and we are trying to go somewhere ? */
	if (tm.getTrainMotionModel().isLost() && state == TrainModel.STATE_RUNNABLE) {
	    if (tm.getPosition(now) != null && !tm.isBlocked()) {
		int newState = setPathToDestination(new ObjectKey(KEY.TRAINS, p, trainIndex), tm);
		if (newState != state)
	...
	case TrainModel.STATE_LOADING:
	    if (tm.getStateLastChangedTime().getTime() + LOADING_DELAY < now.getTime())
		loadTrain(tm, p, trainIndex);
	    return;
	case TrainModel.STATE_UNLOADING:
	    if (tm.getStateLastChangedTime().getTime() + UNLOADING_DELAY < now.getTime()) {
		unloadTrain(trainIndex, p, tm);
		changeWagons(new ObjectKey(KEY.TRAINS, p, trainIndex));
	    }
	    return;
	case TrainModel.STATE_RUNNABLE:
		...
		if (tm.getScheduleIterator().getCurrentOrder(world).unloadTrain) {
		    setState(trainIndex, p, TrainModel.STATE_UNLOADING);
		    return;
		}
		changeWagons(new ObjectKey(KEY.TRAINS, p, trainIndex));
		if (tm.getScheduleIterator().getCurrentOrder(world).loadTrain) {
	...
	NonNullElements j = new NonNullElements(KEY.PLAYERS, world, Player.AUTHORITATIVE);
	...
	while (j.next() && !flag) {
		...
		if (sm.getStationX() == point.x && sm.getStationY() == point.y) {
		    flag = true;
		    break;
		}
	    }
    ...
    private void unloadTrain(int trainIndex, FreerailsPrincipal p, TrainModel tm) {
    ...
    private void loadTrain(TrainModel tm, FreerailsPrincipal p, int trainIndex) {
	ObjectKey trainKey = new ObjectKey(KEY.TRAINS, p, trainIndex);
	ObjectKey stationKey = getStationKey(trainKey, tm);
	
	/* only load the train if there is sufficient cargo at the station */
	if (stationKey != null && dopucmg.checkCargoAtStation(trainKey, stationKey)) {
	...
	TrainModel tm = (TrainModel) world.get(trainKey.key, trainKey.index, trainKey.principal);
	
	ObjectKey stationKey = getStationKey(trainKey, tm);
	if (stationKey != null) {
	    ScheduleIterator si = tm.getScheduleIterator();
	    TrainOrdersModel tom = si.getCurrentOrder(world);
	    Move m = ChangeTrainMove.generateMove(trainKey.index, trainKey.principal, tm,
		    tm.getEngineType(), tom.getConsist());
	    ...
	    StationModel sm = (StationModel) world.get(KEY.STATIONS, stationKey.index,
		...
		if (dest == null) {
		    m = ChangeTrainMove.generateOutOfWaterMove(trainKey, world, false, null, null);
		} else {
		    TrainPath pathToDestination = pathFinder.findPath(dest, head);
		    TrainPathFunction pathFunction = buildPathFunction(pathToDestination, tm, false);
		    m = ChangeTrainMove.generateOutOfWaterMove(trainKey, world, false,
	...
	/*
	 * set new destination if current state is anything other than STOPPED,
	 * and our new state is RUNNABLE
	 */
	if (tm.getState() != TrainModel.STATE_STOPPED && newState == TrainModel.STATE_RUNNABLE) {
	    ObjectKey trainKey = new ObjectKey(KEY.TRAINS, p, trainIndex);
	    // set the trains new destination
	    GameTime t = (GameTime) world.get(ITEM.TIME, Player.AUTHORITATIVE);
	    Move ctdm = ChangeTrainMove.generateMove(trainKey.index, trainKey.principal, tm, tm
		    .getScheduleIterator().nextOrder(world), t);
	...
	ChangeTrainMove m = ChangeTrainMove.generateMove(trainIndex, p, tm, newState,
		(GameTime) world.get(ITEM.TIME, Player.AUTHORITATIVE));
	moveReceiver.processMove(m);
    }
    ...
    private TrainPathSegment getNewSegment(Point p1, Point p2, float t0, float v0, float s0,
	    int mass, EngineType et, boolean outOfWater) {
	float sMax = getSMax(s0, p1, p1);
	return getNewSegment(p1, p2, t0, v0, s0, mass, sMax, et, outOfWater);
    }
    
    /**
     * @return the TrainPathSegment for the traversal from the centre of tile at
     *         point p1 to centre of tile at point p2
     */
    private TrainPathSegment getNewSegment(Point p1, Point p2, float t0, float v0, float s0,
	    int mass, float sMax, EngineType et, boolean outOfWater) {
	float maxTractiveForce = et.getMaxTractiveForce();
	FreerailsTile ft = world.getTile(p1);
	int ttn = ft.getTerrainTypeNumber();
	TerrainType tt1 = (TerrainType) world.get(KEY.TERRAIN_TYPES, ttn, Player.AUTHORITATIVE);
	ft = world.getTile(p2);
	ttn = ft.getTerrainTypeNumber();
	TerrainType tt2 = (TerrainType) world.get(KEY.TERRAIN_TYPES, ttn, Player.AUTHORITATIVE);
	// check the track type to see whether it is a tunnel
	TrackRule trackType = (TrackRule) world.get(KEY.TRACK_RULES, ft.getTrackRule(),
	...
	if (trackType.isTunnel()) {
	    ...
	    effectiveIncline = (float) (tt2.getElevation() - tt1.getElevation());
	    // if we are on a diagonal then our incline is divided by root 2
	    if (p1.x != p2.x && p1.y != p2.y) {
		effectiveIncline /= ROOT_TWO;
	    }
	    effectiveIncline += (float) (tt1.getRoughness() + tt2.getRoughness()) / 2;
    ...
    private TrainPathFunction buildPathFunction(TrainPath tp, TrainModel tm, boolean outOfWater) {
	...
	float s0 = 0;
	int tBase = ((GameTime) world.get(ITEM.TIME, Player.AUTHORITATIVE)).getTime();
	float t0 = 0.0f;
	int mass = trainModelViewer.getTotalMass();
	EngineType et = (EngineType) world.get(KEY.ENGINE_TYPES, tm.getEngineType(),
		Player.AUTHORITATIVE);
	...
	if (tm.getTrainMotionModel().getPathFunction() != null)
	    v0 = tm.getTrainMotionModel().getPathFunction().getSpeed(tBase);
	...
	if (p1.equals(p2)) {
	    ...
	    TrainPathSegment seg = getNewSegment(p1, p2, t0, v0, s0, mass, (float) il.getLength()
		    .getLength(), et, outOfWater);
	    segList.add(seg);
	    t0 = seg.getTMax();
	    if (t0 < 0) {
	...
	for (int i = mapCoords.size() - 1; i >= 0; i--) {
	    ...
	    float currentS0 = s0;
	    do {
		TrainPathSegment seg = getNewSegment(p1, p2, t0, v0, currentS0, mass, s0
			+ (sMax - s0) * ((float) (currentChunk + 1) / maxChunks), et, outOfWater);
		float newT0;
		newT0 = seg.getTMax();
		float vDiff = Math.abs(seg.getSpeed(newT0) - seg.getSpeed(t0));
		float a = Math.abs(seg.getAcceleration());
		if (prefChunks <= maxChunks && (newT0 < t0 || (vDiff > a * 4 && vDiff > 0.0001))) {
		    ...
		    continue;
		} else if (prefChunks >= maxChunks && (currentChunk % 2) == 0 && maxChunks > 1
			&& vDiff <= a) {
		    // try larger chunks
		    maxChunks >>= 1;
		    currentChunk >>= 1;
	...
	if (timeOfLastAttempt != null) {
	    GameCalendar gc = (GameCalendar) world.get(ITEM.CALENDAR, Player.AUTHORITATIVE);
	    if (timeOfLastAttempt.intValue() + gc.getTicksPerDay() > t.getTime())
		return tm.getState();
	}
	...
	if (stationCoords == null) {
	    /* no orders */
	    ChangeTrainMove ctm = ChangeTrainMove.generateMove(trainKey.index, trainKey.principal,
		    tm, (TrainPath) null, null, t);
	...
	TrainPathFunction tpf = buildPathFunction(tp, tm, tm.getTrainMotionModel().isOutOfWater());
	ChangeTrainMove ctm;
	if (tpf == null) {
	    ...
	    ctm = ChangeTrainMove.generateMove(trainKey.index, trainKey.principal, tm, tp, tpf, t);
	...
	GameTime now = (GameTime) world.get(ITEM.TIME, Player.AUTHORITATIVE);
	TrainOrdersModel tom = tm.getScheduleIterator().getCurrentOrder(world);
	if (tom == null)
	    return null;
	
	ObjectKey stationKey = tom.getStationNumber();
	StationModel station = (StationModel) world.get(stationKey.key, stationKey.index,
		stationKey.principal);
	...
	if (!isOutOfWater && trainModelViewer.getWaterRemaining() == 0) {
	    ...
	    Move m;
	    if (dest == null) {
		m = ChangeTrainMove.generateOutOfWaterMove(trainKey, world, true, null, null);
	    } else {
		TrainPath newPathToDestination = pathFinder.findPath(dest, head);
		TrainPathFunction pathFunction = buildPathFunction(newPathToDestination, train,
			true);
		if (pathFunction == null) {
		    m = ChangeTrainMove.generateOutOfWaterMove(trainKey, world, true, null, null);
		} else {
		    m = ChangeTrainMove.generateOutOfWaterMove(trainKey, world, true,
			    newPathToDestination, pathFunction);
		}
	    }
	    moveReceiver.processMove(m);
	    return true;
