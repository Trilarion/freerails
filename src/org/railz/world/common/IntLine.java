...
public class IntLine implements FreerailsSerializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4508003253013291922L;
	/**
	...
	public byte getDirection() {
		...
		if (dx > 0) {
			...
			return (dy < 0) ? CompassPoints.NORTHWEST
					: ((dy > 0) ? CompassPoints.SOUTHWEST : CompassPoints.WEST);
		}
		return (dy < 0) ? CompassPoints.NORTH : ((dy > 0) ? CompassPoints.SOUTH
				: 0x0);
	}
	...
	public void append(IntLine l) {
		assert (l.getDirection() == getDirection() || (l.x1 == l.x2 && l.y1 == l.y2))
				&& (l.x1 == x2 && l.y1 == y2);
		x2 = l.x2;
		y2 = l.y2;
		length.add(l.length);
	...
	public void prepend(IntLine l) {
		if (!((l.getDirection() == getDirection()) || (l.x1 == l.x2 && l.y1 == l.y2))
				&& (l.x2 == x1 && l.y2 == y1)) {
