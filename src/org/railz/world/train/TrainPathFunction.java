...
public class TrainPathFunction implements FreerailsSerializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2266107249579090260L;
	...
	public TrainPathFunction(int tBase, ArrayList segmentArrayList) {
		this.tBase = tBase;
		segments = (TrainPathSegment[]) segmentArrayList
				.toArray(new TrainPathSegment[segmentArrayList.size()]);
	}
	...
	public float getSpeed(int t) {
		...
		for (int i = 0; i < segments.length; i++) {
			if (segments[i].tMax >= t)
				return segments[i].getSpeed(t);
		}
		return segments[segments.length - 1]
				.getSpeed(segments[segments.length - 1].tMax);
	}
	...
	public float getDistance(int t) {
		...
		for (int i = 0; i < segments.length; i++) {
			if (segments[i].tMax >= t)
				return segments[i].getDistance(t);
		}
		return segments[segments.length - 1]
				.getDistance(segments[segments.length - 1].tMax);
	}
	...
	public static class TrainPathSegment implements FreerailsSerializable {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -3811667586640443093L;
		...
		public TrainPathSegment(float t0, float v0, float a, float s0,
				float sMax) {
			this.t0 = t0;
			this.v0 = v0;
			...
			if (a == 0.0f) {
				...
				tMax = t0
						+ (-v0 + (float) Math.sqrt(v0 * v0 + 2.0f * a * deltaS))
						/ a;
			}
		}
		...
		public String toString() {
			return "Segment: t0=" + t0 + ", v0=" + v0 + ", a=" + a + ", s0="
					+ s0 + ", tMax=" + tMax;
		}
