...
final public class EngineType implements FreerailsSerializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3539538416358521948L;
	...
	public EngineType(String name, long m, long maintenance,
			int annualFuelConsumption, int fuelType, int waterCapacity,
			int mass, int powerOutput, int maxTractiveForce,
	...
	/**
	 * @param effectiveIncline
	 *            effectiveIncline in %
	 * @return acceleration at the given speed and incline
	 */
	public float getAcceleration(float effectiveIncline, float v,
			int totalMass, boolean outOfWater) {
		float tractiveForce = powerOutput / (outOfWater ? 3 * v : v);
		// at low speeds, acceleration is limited by traction
		if (tractiveForce > maxTractiveForce)
			tractiveForce = maxTractiveForce;
		...
		// 100
		return tractiveForce / totalMass - effectiveIncline * 0.098f - (v * v)
				* dragCoefficient / (totalMass) - v
				* rollingFrictionCoefficient;
	}

	/**
	 * @param mass
	 *            of train + engine in tonnes
	 * @param effectiveIncline
	 *            effective incline in %
	 * @return maximum speed in deltas per tick
	 */
	public float getMaxSpeed(float effectiveIncline, int mass) {
		float increment = (5 * EngineType.TILE_HOURS_PER_MILE_BIGTICKS * TrackTile.DELTAS_PER_TILE)
				/ GameTime.TICKS_PER_BIG_TICK;
		float vMin = 0;
		float vMax = 0;
		float aMin, aMax;
		do {
