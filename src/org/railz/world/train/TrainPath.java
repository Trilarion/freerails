...
public final class TrainPath implements FreerailsSerializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4757794733273054818L;
	...
	/**
	 * This is the intended length of the path that we attempt to maintain when
	 * moving the head or tail. we store the path length in order to avoid
	 * changes in total length caused by rounding errors when summing calculated
	 * diagonals
	 */
	...
	public TrainPath(IntLine[] lines) {
		...
		for (int i = 0; i < lines.length; i++) {
			if (lines[i].getLength().getLength() == 0.0 && i < lines.length - 1)
				continue;
	...
	/**
	 * Append the specified path to the tail. Actual total lengths is preserved
	 * and intended length is calculated.
	 */
	public void append(TrainPath tp) {
		IntLine tpHead = (IntLine) tp.segments.getFirst();
		IntLine tail = (IntLine) segments.getLast();
		if (tpHead.x1 != tail.x2 || tpHead.y1 != tail.y2)
			throw new IllegalArgumentException();
	...
	public TrainPath truncateTail(double newLength) {
		...
		if ((int) actualLength.getLength() <= newLength) {
			...
			return new TrainPath(
					new IntLine[] { new IntLine(p.x, p.y, p.x, p.y) });
		}
		...
		if ((int) l.getLength() > newLength) {
			...
			removedSegments
					.addFirst(new IntLine(line.x2, line.y2, oldX2, oldY2));
		}
		length = newLength;
		actualLength = l;
		...
		return new TrainPath(
				(IntLine[]) removedSegments.toArray(new IntLine[removedSegments
						.size()]));
	}

	/**
	 * Prepend the specified path to this TrainPath. The tail of additionalPath
	 * is equal to the head of this path
	 */
	public void prepend(TrainPath additionalPath) {
		...
		ListIterator i = additionalPath.segments
				.listIterator(additionalPath.segments.size() - 1);
		while (i.hasPrevious()) {
	 ...
	 *            the path to add. The tail of additionalPath is equal to the
	 *            head of this path.
	 ...
	 * 
	 * @param additionalPath
	 *            the path to add. The head of additionalPath is equal to the
	 *            tail of this path.
	 ...
	 * 
	 * @param mapCoords
	 *            an empty HashMap into which the map coordinates are to be put
	 *            as keys, and the directions as objects.
	 */
			...
			do {
				...
				if (dx < 0) {
					...
					direction = (byte) (CompassPoints.NORTHWEST
							| CompassPoints.WEST | CompassPoints.SOUTHWEST);
				} else if (dx > 0) {
					map.x++;
					direction = (byte) (CompassPoints.NORTHEAST
							| CompassPoints.EAST | CompassPoints.SOUTHEAST);
				} else {
					direction = (byte) (CompassPoints.NORTH | CompassPoints.SOUTH);
				}
				if (dy < 0) {
					map.y--;
					direction &= (byte) (CompassPoints.NORTHWEST
					...
					direction &= (byte) (CompassPoints.SOUTHWEST
							| CompassPoints.SOUTH | CompassPoints.SOUTHEAST);
				} else {
					direction &= (byte) (CompassPoints.EAST | CompassPoints.WEST);
				}
	 ...
	 * @param p
	 *            used to return the absolute position of the locus measured in
	 *            Deltas from the map origin.
	 * @param distance
	 *            distance of the locus from the head of the TrainPath
	 */
	public byte getDirectionAtDistance(Point p, int distance) {
		...
		while (li.hasNext()) {
			...
			if (d.getLength() >= distance) {
				...
				pl.setLength(l.getLength().getLength()
