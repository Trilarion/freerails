...
public class TrainModel implements FreerailsSerializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2357461756862111368L;

	/**
	 * Path length traversed since we last loaded water, as of the last resync
	 */
	...
	/**
	 * The number of ticks this train has been in the RUNNABLE state, at the
	...
	public String toString() {
		...
		return "TrainModel " + super.toString() + ": stateLastChanged = "
				+ stateLastChanged + ", scheduleIterator = " + scheduleIterator
				+ ", state = " + state + ", isBlocked=" + isBlocked + ", TMM: "
				+ trainMotionModel + ", engineType=" + engineType + ", wagons="
				+ s + ", cargoBundleNo=" + cargoBundleNumber + ", priority="
				+ priority + ", ticksInService=" + ticksInService + ", ptslw="
	 ...
	 *            type of the engine
	 * @param wagons
	 *            array of indexes into the WAGON_TYPES table
	 * @param bundleId
	 *            index into the CARGO_BUNDLES table
	 * @param creationDate
	 *            time the train was created
	 */
	public TrainModel(int engine, int[] wagons, int bundleId,
			GameTime creationDate) {
		this(engine, wagons, bundleId, creationDate, STATE_UNLOADING, null,
				null, PRIORITY_NORMAL, false, creationDate, 0, 0);
	}
	...
	public TrainModel(TrainModel trainModel, TrainPath pathToDestination,
		...
		this(trainModel.engineType, trainModel.wagonTypes,
				trainModel.cargoBundleNumber, trainModel.creationDate,
				trainModel.state, trainModel.scheduleIterator, null,
		...
		if (trainMotionModel != null)
			costTraversedSinceLoadingWater += trainMotionModel
					.getCostTraversed(now);
	...
	public TrainModel(TrainModel tm, ScheduleIterator si, GameTime t) {
		this(tm.engineType, tm.wagonTypes, tm.cargoBundleNumber,
				...
				tm.trainMotionModel == null ? null : tm.trainMotionModel
						.clearPathToDestination(t), tm.priority, tm.isBlocked,
				tm.stateLastChanged, tm.ticksInService, tm
						.getCostTraversedSinceLoadingWater(t));
	}
	...
	public TrainModel(TrainModel tm, GameTime now, int state) {
		this(tm.engineType, tm.wagonTypes, tm.cargoBundleNumber,
				...
				(tm.trainMotionModel == null ? null : tm.trainMotionModel
						.clearPathToDestination(now)), tm.priority, false, now,
				tm.state == STATE_RUNNABLE ? tm.ticksInService + now.getTime()
						- tm.stateLastChanged.getTime() : tm.ticksInService, tm
						.getCostTraversedSinceLoadingWater(now));
	}

	/**
	 * copy constructor with original schedule, cargo, position, but new engine
	 * and wagons
	 */
	public TrainModel getNewInstance(int newEngine, int[] newWagons) {
		TrainModel tm = new TrainModel(newEngine, newWagons,
				...
				costTraversedSinceLoadingWater);
		tm.trainMotionModel = tm.trainMotionModel.setTrainPathLength(tm
				.getLength());
		return tm;
	}
	...
	private TrainModel(int engine, int[] wagons, int bundleId,
			GameTime creationDate, int state,
			ScheduleIterator scheduleIterator, TrainMotionModel2 motionModel,
			int priority, boolean isBlocked, GameTime stateLastChanged,
			long ticksInService, int costTraversedSinceLoadingWater) {
		...
		trainMotionModel = motionModel == null ? null : new TrainMotionModel2(
				motionModel);
	...
	/**
	 * @return train length in Deltas. The server guarantees to the client that
	 *         the length of the train in tiles will not be less than the number
	 *         of wagons + 1.
	 */
	...
	public boolean equals(Object obj) {
		if (obj instanceof TrainModel) {
			...
			boolean b = this.cargoBundleNumber == test.cargoBundleNumber
					...
					&& (scheduleIterator != null ? this.scheduleIterator
							.equals(test.scheduleIterator)
					...
					&& costTraversedSinceLoadingWater == test.costTraversedSinceLoadingWater;
	...
	public TrainModel setBlocked(boolean blocked, GameTime now) {
		return new TrainModel(engineType, wagonTypes, cargoBundleNumber,
				...
				trainMotionModel == null ? null : trainMotionModel
						.clearPathToDestination(now), priority, blocked,
				stateLastChanged, ticksInService,
				getCostTraversedSinceLoadingWater(now));
	}

	public TrainPath getPosition(GameTime t) {
		return trainMotionModel == null ? null : trainMotionModel
				.getPosition(t);
	}

	public TrainModel setPosition(TrainPath position, GameTime t) {
		return new TrainModel(engineType, wagonTypes, cargoBundleNumber,
				creationDate, state, scheduleIterator, new TrainMotionModel2(
						null, position, t, null), priority, isBlocked,
				stateLastChanged, ticksInService,
				costTraversedSinceLoadingWater);
	}
	...
	public TrainModel loadWater(GameTime t0, boolean empty,
		...
		return new TrainModel(engineType, wagonTypes, cargoBundleNumber,
				creationDate, state, scheduleIterator, tmm, priority,
