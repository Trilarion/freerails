...
import org.railz.move.AddRemoveScheduleStationMove;
...
public class TrainScheduleJPanel extends javax.swing.JPanel implements
	...
	private static final String CLASS_NAME = TrainScheduleJPanel.class
			.getName();
	...
	private final ModdableResourceFinder graphicsResourceFinder;
	...
	private void initComponents() {// GEN-BEGIN:initComponents
		...
		gotoStationJMenuItem.setText(org.railz.util.Resources
				.get("Go to station"));
		...
		removeStationJMenuItem.setText(org.railz.util.Resources
				.get("Remove station"));
		...
		changeConsistJMenuItem.setText(org.railz.util.Resources
				.get("Change Consist"));
		changeConsistJMenuItem
				.addActionListener(new java.awt.event.ActionListener() {
					@Override
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		waitUntilFullJMenuItem.setText(org.railz.util.Resources
				.get("Wait until full"));
		...
		addStationJButton.setIcon(new ImageIcon(graphicsResourceFinder
				.getURLForReading("toolbar/add_station.png")));
		addStationJButton.setToolTipText(org.railz.util.Resources
				.get("Add station"));
		addStationJButton
				.addActionListener(new java.awt.event.ActionListener() {
					@Override
		...
		setBorder(new javax.swing.border.TitledBorder(
				org.railz.util.Resources.get("Schedule")));
	...
	private void changeConsistJMenuItemActionPerformed(
			java.awt.event.ActionEvent evt) {// GEN-FIRST:event_changeConsistJMenuItemActionPerformed
		...
		int[] orderWagons = ((TrainOrdersListElement) listModel
				.getElementAt(wagonSelectionOrderNo)).order.getConsist();
		tm = tm.getNewInstance(tm.getEngineType(), orderWagons);
		wagonSelectionJPanel.setup(modelRoot, tm);
		wagonSelectionDialog = guiRoot.getDialogueBoxController().createDialog(
				wagonSelectionJPanel, Resources.get("Select new consist"));
	...
	private void addStationJButtonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_addStationJButtonActionPerformed
		...
		Schedule s = (Schedule) w.get(KEY.TRAIN_SCHEDULES, tm
				.getScheduleIterator().getScheduleKey().index, tm
				.getScheduleIterator().getScheduleKey().principal);
		TrainOrdersModel order = new TrainOrdersModel(new ObjectKey(
				KEY.STATIONS, modelRoot.getPlayerPrincipal(), 0), consist,
				false, true, true);
	...
	private void gotoStationJMenuItemActionPerformed(
		...
		Move m = ChangeTrainMove.generateMove(
				trainNumber,
				modelRoot.getPlayerPrincipal(),
				(TrainModel) w.get(KEY.TRAINS, trainNumber,
						modelRoot.getPlayerPrincipal()), si,
	...
	private void pushDownJMenuItemActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_pushDownJMenuItemActionPerformed
		...
		ImmutableSchedule immutableSchedule = (ImmutableSchedule) w.get(
				KEY.TRAIN_SCHEDULES, train.getScheduleIterator()
						.getScheduleKey().index, train.getScheduleIterator()
						.getScheduleKey().principal);
		int i = orders.getSelectedIndex();
	...
	private final PortablePopupAdapter ordersPopupAdapter = new PortablePopupAdapter() {
		@Override
		public void triggerPopup(MouseEvent e) {
			...
			if (-1 != i) {
				...
				editOrderJPopupMenu.show(e.getComponent(), e.getX(), e.getY());
			}
		}
	};
	...
	private void setup(ModelRoot mr, GUIRoot gr) {
		...
		// This actionListener is fired by the select station popup when a stion
		// is selected.
	...
	public void display(int trainNumber) {
		...
		if (wagonSelectionDialog != null && this.trainNumber != trainNumber) {
		...
		if (trainNumber >= 0) {
			...
			if (scheduleIterator == null) {
				LOGGER.logp(Level.SEVERE, CLASS_NAME, METHOD_NAME,
						"Schedule Iterator is null !!!!!!");
			} else {
				LOGGER.logp(Level.FINE, CLASS_NAME, METHOD_NAME,
						"Schedule Iterator is fine");
			}
	...
	void sendUpdateStationMove(int aStationIndex, TrainOrdersModel aOrder) {
		Move m = AddRemoveScheduleStationMove.generateChangeMove(
				new ObjectKey(KEY.TRAIN_SCHEDULES, scheduleIterator
						.getScheduleKey().principal, scheduleIterator
						.getScheduleKey().index), aStationIndex, aOrder, w);
		modelRoot.getReceiver().processMove(m);
	}
	...
	void sendRemoveStationMove(int aStationIndex) {
		TrainModel train = (TrainModel) w.get(KEY.TRAINS, trainNumber,
				modelRoot.getPlayerPrincipal());
		Move m = AddRemoveScheduleStationMove.generateRemoveMove(new ObjectKey(
				KEY.TRAIN_SCHEDULES, train.getScheduleIterator()
						.getScheduleKey().principal, train
						.getScheduleIterator().getScheduleKey().index),
				aStationIndex, w);
		modelRoot.getReceiver().processMove(m);
	}
	...
	void sendAddStationMove(int aStationIndex, TrainOrdersModel aOrder) {
		TrainModel train = (TrainModel) w.get(KEY.TRAINS, trainNumber,
				modelRoot.getPlayerPrincipal());
		Move m = AddRemoveScheduleStationMove.generateAddMove(new ObjectKey(
				KEY.TRAIN_SCHEDULES, train.getScheduleIterator()
						.getScheduleKey().principal, train
						.getScheduleIterator().getScheduleKey().index),
	...
	public void listUpdated(KEY key, int index, FreerailsPrincipal p) {
		...
		if (KEY.TRAIN_SCHEDULES == key) {
			...
			if (scheduleIterator != null) {
				...
				LOGGER.logp(Level.SEVERE, CLASS_NAME, METHOD_NAME,
						"scheduleIterator is null. Known bug for change carriage consistancy.");
			}
			...
			if (temp.index == index && temp.principal.equals(p)) {
				condition1 = true;
			}
	...
	private final ComponentListener changeConsistListener = new ComponentAdapter() {
		@Override
		public void componentHidden(ComponentEvent e) {
			int[] wagons = wagonSelectionJPanel.getWagons();
			TrainOrdersModel oldOrder = ((TrainOrdersListElement) listModel
					.getElementAt(wagonSelectionOrderNo)).order;
