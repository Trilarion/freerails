...
/**
 * A move executer which pre-commits moves on the outward trip to the server,
 * and rolls back rejected moves when received back from the server.
 * 
 * TODO sort out "undo moves" mess.
 * 
 * Requirements for Undo Move: *Move must already have been performed *Undo must
 * not succeed if circumstances have changed such that the undo is not possible.
 * *Undo cannot be performed more than once *Client must receive notification of
 * change. *Order of composite undo must be reversed wrt forwards move. *Undo
 * must only be executable by client which performed the move. *Undos must
 * remain in stack for sufficient time for human to manually perform them
 * 
 * @author rtuck99@users.sourceforge.net
 */
public class NonAuthoritativeMoveExecuter implements UncommittedMoveReceiver, GameModel {
    ...
    private static final String CLASS_NAME = NonAuthoritativeMoveExecuter.class.getName();
    private static final Logger logger = LogManager.getLogger(CLASS_NAME);
    
    public NonAuthoritativeMoveExecuter(World w, MoveReceiver mr, ModelRoot modelRoot) {
	this.modelRoot = modelRoot;
	moveReceiver = mr;
	world = w;
	modelRoot.getDebugModel().getClientMoveDebugModel().getAction()
		.addPropertyChangeListener(debugChangeListener);
    }
     ...
     *         pre-committed. New moves are added at the end, and removed from
     *         the front when committed.
     */
	...
	private boolean undoMoves() {
		...
		if (ms == MoveStatus.MOVE_OK) {
		    ...
		    logger.log(Level.INFO, "Unrolled precommited move " + attempted.toString());
		    rejectedMoves.remove(i);
		    forwardMove(new UndoneMove(attempted), ms);
		    n++;
		} else {
		    logger.log(Level.WARNING, "FAILED to undo move " + attempted + " because " + ms);
		    assert false;
		}
	    }
	...
	private void moveCommitted(Move move) {
		...
		if (move instanceof RejectedMove) {
			...
			logger.log(Level.INFO,
				"Logging rejected move " + ((RejectedMove) move).getAttemptedMove());
			/*
			...
			do {
				...
				if (am.doMove(world, am.getPrincipal()) != MoveStatus.MOVE_OK) {
				    break;
				}
				
				logger.log(Level.INFO, "Committed queued move " + am);
		    ...
		    logger.log(Level.FINE, "Precommitted move acknowledged " + "by server:" + move);
		    // move succeeded and we have already executed it
		    return;
		}
		// move succeeded but was submitted by another client
		logger.log(Level.FINE, "Didn't recognise move: " + move + "<=>" + pendingMove);
	    }
		...
		if (ms != MoveStatus.MOVE_OK) {
			...
			logger.log(Level.INFO, "Dropping redundant move " + "received from server:"
				+ move);
			return;
		    }
		    
		    logger.log(Level.INFO, "Queueing blocked move " + move + " - " + ms.toString());
		    
		    /*
		     * move could not be committed because of a pre-commited
		     * move yet to be rejected by the server
		     */
		...
		logger.log(Level.FINE, "received unknown rejected move!" + move);
	    }
	}
    ...
    private PropertyChangeListener debugChangeListener = new PropertyChangeListener() {
	public void propertyChange(PropertyChangeEvent e) {
	    if (modelRoot.getDebugModel().getClientMoveDebugModel().isSelected()) {
