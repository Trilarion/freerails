 ...
 */
public class ConnectionAdapter implements UntriedMoveReceiver, ConnectionListener {
    ...
    public ConnectionAdapter(ModelRoot mr, GUIRoot gr, Player player, FreerailsProgressMonitor pm,
	    GUIClient gc) {
	modelRoot = mr;
	this.player = player;
	...
	modelRoot.getUserMessageLogger().println(Resources.get("Connection to server closed"));
    }
    
    public synchronized void setConnection(ConnectionToServer c) throws IOException,
     ...
     * launcher.
     */
    private synchronized void setConnectionImpl(ConnectionToServer c) throws IOException,
	    GeneralSecurityException {
	if (gameLoop != null) {
	    gameLoop.stop();
	}
	...
	modelRoot.getUserMessageLogger().println(
		Resources.get("Attempting to authenticate player: ") + player.getName());
	authenticated = false;
	...
	synchronized (worldUpdater) {
	    ...
	    moveExecuter = new NonAuthoritativeMoveExecuter(world, moveReceiver, modelRoot);
	...
	try {
	    ...
	    ViewLists viewLists = new ViewListsImpl(modelRoot, guiRoot, progressMonitor);
		 ...
		 */
		modelRoot.getUserMessageLogger().println(
			Resources.get("Your client is not compatible with " + "the server."));
	    }
	    ...
	    String s = Resources.get("There was a problem reading in the graphics " + "data");
	...
	if (s instanceof AddPlayerResponseCommand) {
		...
		if (authenticated) {
		    modelRoot.getUserMessageLogger().println(
			    Resources.get("Player was successfully authenticated"));
		...
		Resources.setExternalResourceBundle(response.getResourceBundle());
	    } else {
		logger.log(Level.WARNING, "Couldn't get resource bundle from " + "server");
	    }
	}
    }
	...
	public void run() {
		...
		modelRoot.getUserMessageLogger().println(
			Resources.get("Unable to authenticate with server: ") + e.toString());
	    }
	}
    }
}
