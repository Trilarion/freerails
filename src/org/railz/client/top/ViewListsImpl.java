...
public class ViewListsImpl implements ViewLists {
    ...
    public ViewListsImpl(ModelRoot mr, GUIRoot gr, FreerailsProgressMonitor pm) throws IOException {
	guiRoot = gr;
	mrf = guiRoot.getGraphicsResourceFinder();
	modelRoot = mr;
	ReadOnlyWorld w = mr.getWorld();
	
	imageManager = new ImageManagerImpl(guiRoot.getClientJFrame(), "org/railz/client/graphics/");
    ...
    private TileRendererList loadBuildingRenderers(ReadOnlyWorld w, FreerailsProgressMonitor pm)
	    throws IOException {
	...
	NonNullElements i = new NonNullElements(KEY.BUILDING_TYPES, w, Player.AUTHORITATIVE);
	while (i.next()) {
	    ...
	    try {
		StandardTileRenderer tr = new StandardTileRenderer(imageManager, tileTypes,
			buildingType);
	    ...
	    try {
		ChequeredTileRenderer tr = new ChequeredTileRenderer(imageManager, buildingType,
			tileTypes);
		buildingRenderers.add(tr);
		continue;
	    } catch (IOException e) {
	    ...
	    try {
		StationRenderer sr = new StationRenderer(imageManager, buildingType);
		buildingRenderers.add(sr);
		continue;
	    } catch (IOException e) {
		// no more renderers to try
		pm.setMessage(Resources.get("Problem loading building " + "graphics"));
		throw e;
	    }
	}
    ...
    private TileRendererList loadTerrainRenderers(ReadOnlyWorld w, FreerailsProgressMonitor pm)
	    throws IOException {
	...
	for (int i = 0; i < numberOfTypes; i++) {
	    TerrainType t = (TerrainType) w.get(KEY.TERRAIN_TYPES, i, Player.AUTHORITATIVE);
		...
		logger.log(Level.WARNING, "No tile renderer for " + t.getTerrainTypeName());
	    }
	}
	...
	for (int j = 0; j < numberOfTypes; j++) {
	    TerrainType t2 = (TerrainType) w.get(KEY.TERRAIN_TYPES, j, Player.AUTHORITATIVE);
	...
	for (int j = 0; j < numberOfTypes; j++) {
		...
		TerrainType t = (TerrainType) w.get(KEY.TERRAIN_TYPES, j, Player.AUTHORITATIVE);
		TileRenderer tr = new SpecialTileRenderer(imageManager, new int[] { j }, t,
			occeanTileRenderer);
	...
	if (key.key == KEY.STATION_IMPROVEMENTS) {
	    StationImprovement si = (StationImprovement) modelRoot.getWorld().get(key.key,
