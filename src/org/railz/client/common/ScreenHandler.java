 ...
 * 
 * <h1>Screen redraw operation</h1> Screen redraws may be caused by the
 * following events: <h2>periodic frame update</h2> This occurs when the
 * update() method is called. The main map display is redrawn, plus any
 * components which may be obscuring it. <h2>A refresh is caused by the event
 * thread (eg. the user clicks a button)</h2> Client code should cause the
 * relevant parts to be redrawn. <h2>A refresh is caused by a world model change
 * </h2> Client code should call repaint() for the changed components.
 * 
 * <p>
 * Note that redraws are inefficient at the moment due to differing platform
 * behaviour, e.g. OS X provides native double buffering so buffering in Java is
 * not required. X11 redraws should be double buffered by Java, but there is as
 * yet no mechanism to sync the two buffers without doing a full redraw which is
 * wasteful.
 */
final public class ScreenHandler {
    ...
    private boolean runningOSX = "Mac OS X".equals(System.getProperty("os.name"));
    
    private static final String CLASS_NAME = ScreenHandler.class.getName();
    ...
    public ScreenHandler(JFrame f, UpdatedComponent uc, int mode, DisplayMode displayMode) {
	updatedComponent = uc;
	...
	for (int i = 0; i < dirtyComponents.length; i++) {
		...
		RepaintManager.currentManager(dirtyComponents[i]).markCompletelyDirty(
			(JComponent) dirtyComponents[i]);
	    }
	}
    }
	...
	public void run() {
		...
		logger.log(Level.INFO, "isFullScreenRequired = " + bc.isFullScreenRequired());
		logger.log(Level.INFO, "isPageFlipping = " + bc.isPageFlipping());
		ImageCapabilities ic = bc.getBackBufferCapabilities();
		logger.log(Level.INFO, "isAccelerated = " + ic.isAccelerated());
		logger.log(Level.INFO, "isTrueVolatile = " + ic.isTrueVolatile());
		logger.log(Level.INFO, "isMultiBufferAvailable = " + bc.isMultiBufferAvailable());
	...
	case ScreenHandler.FULL_SCREEN:
	    JInternalFrame jif = new JInternalFrame(title, true);
	...
	default:
	    ...
	    jd.pack();
	    jd.setLocation(frame.getX() + (frame.getWidth() - jd.getWidth()) / 2, frame.getY()
