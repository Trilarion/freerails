...
final public class MapBackgroundRender implements MapLayerRenderer {
    private static final String CLASS_NAME = MapBackgroundRender.class.getName();
    ...
    /*
     * Used to avoid having to create a new rectangle for each call tothe paint
     * methods.
     */
	...
	public void paintRectangleOfTiles(Graphics g, Rectangle tilesToPaint) {
		...
		for (tile.y = tilesToPaint.y - 1; tile.y < (tilesToPaint.y + tilesToPaint.height + 1); tile.y++) {
			...
			TrackTile tp = (TrackTile) w.getTile(tile.x, tile.y).getTrackTile();
			if (tp == null)
			    continue;
			byte trackConfig = tp.getTrackConfiguration();
			int ruleNumber = tp.getTrackRule();
			TrackPieceRenderer trackPieceView = trackPieceViewList
				.getTrackPieceView(ruleNumber);
			trackPieceView.drawTrackPieceIcon(trackConfig, tempG, tile.x, tile.y,
				TileRenderer.TILE_SIZE);
		    }
		}
	    }
	}
	...
	public TrackLayer(ReadOnlyWorld world, TrackPieceRendererList trackPieceViewList) {
	...
	public void paintTile(Graphics g, Point tile) {
		...
		if (null == tr) {
		    logger.log(Level.SEVERE, "No tile renderer for " + typeNumber);
		} else {
	 ...
	 *            The grahics context.
	 * @param tilesToPaint
	 *            The rectangle defining the area of the map to draw,
	 *            measured in tile coordinates, to paint.
	 */
	public void paintRectangleOfTiles(Graphics g, Rectangle tilesToPaint) {
	    Point tile = new Point();
	...
	/**
	 * @param g
	 *            graphics context on which to draw, origin at top left of
	 *            map origin
	 * @param x
	 *            map coord in tiles of area to draw
	 * @param y
	 *            map coord in tiles of area to draw
	 * @param width
	 *            width in tiles of area to draw
	 * @param height
	 *            height in tiles of area to draw
	 */
	private void paintRectangleOfTiles(Graphics g, int x, int y, int width, int height) {
	    paintRectangleOfTiles(g, new Rectangle(x, y, width, height));
	}
    ...
    /**
     * @param g
     *            Graphics context with origin pointing to top left corner of
     *            viewport.
     * @param visibleRect
     *            rectangle defining area of map to draw relative to origin 0,0
     *            at top left of map, measured in pixels.
     */
    public void paintRect(Graphics g, Rectangle visibleRect) {
	int x = visibleRect.x / TileRenderer.TILE_SIZE.width;
	int y = visibleRect.y / TileRenderer.TILE_SIZE.height;
	int width = (visibleRect.width + visibleRect.x - 1) / TileRenderer.TILE_SIZE.width - x + 1;
	int height = (visibleRect.height + visibleRect.y - 1) / TileRenderer.TILE_SIZE.height - y
		+ 1;
    ...
    /**
     * @param g
     *            graphics context on which to draw, origin at top left of area
     *            to draw
     * @param x
     *            map coord in tiles of area to draw
     * @param y
