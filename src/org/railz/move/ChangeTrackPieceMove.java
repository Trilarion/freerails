...
final public class ChangeTrackPieceMove implements TrackMove, MapUpdateMove {
    ...
    private MoveStatus tryMove(World w, TrackTile oldTrackPiece, TrackTile newTrackPiece,
	...
	FreerailsTile currentTile = (FreerailsTile) w.getTile(location.x, location.y);
	...
	if ((oldTrackPiece == null && currentTrackPieceAtLocation != null)
		|| ((oldTrackPiece != null) && !oldTrackPiece.equals(currentTrackPieceAtLocation))) {
	    return MoveStatus.moveFailed("Somebody else changed the track " + "piece.");
	}
	
	// Check that the track is not locked by a train
	if (currentTrackPieceAtLocation != null && currentTrackPieceAtLocation.isLocked()) {
	...
	if (oldTrackPiece != null) {
	    ...
	    if (!oldTrackRule.testTrackPieceLegality(oldTrackPiece.getTrackConfiguration()))
	...
	if (newTrackPiece != null) {
	    ...
	    if (!newTrackRule.testTrackPieceLegality(newTrackPiece.getTrackConfiguration()))
		return MoveStatus.moveFailed("Illegal track " + "configuration.");
	    TerrainType tt = (TerrainType) w.get(KEY.TERRAIN_TYPES,
		    currentTile.getTerrainTypeNumber(), Player.AUTHORITATIVE);
	    if (!newTrackRule.canBuildOnThisTerrainType(tt.getTerrainCategory())) {
		String thisTrackType = newTrackRule.toString();
		int terrainCategory = tt.getTerrainCategory();
		
		return MoveStatus.moveFailed("Can't build " + thisTrackType + " on "
			+ terrainCategory);
	    }
	    BuildingTile bTile = currentTile.getBuildingTile();
	    if (bTile != null) {
		BuildingType bType = (BuildingType) w.get(KEY.BUILDING_TYPES, bTile.getType(),
			Player.AUTHORITATIVE);
		if (!bType.isTrackLayoutValid(newTrackPiece.getTrackConfiguration()))
    ...
    private void move(World w, TrackTile oldTrackPiece, TrackTile newTrackPiece,
	    FreerailsPrincipal p) {
	...
	if (o instanceof ChangeTrackPieceMove) {
	    ...
	    boolean fieldoldTrackPieceEqual = ((trackPieceBefore == null && m.trackPieceBefore == null) || (trackPieceBefore != null && trackPieceBefore
		    .equals(m.trackPieceBefore)));
	    boolean fieldnewTrackPieceEqual = ((trackPieceAfter == null && m.trackPieceAfter == null) || (trackPieceAfter != null && trackPieceAfter
	...
	return "ChangeTrackPieceMove: before=" + trackPieceBefore + ", after=" + trackPieceAfter
		+ ", owner=" + trackOwner + ", location=" + location;
    }
}
