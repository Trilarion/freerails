...
public class TrackRenderer {
     ...
     * @return
     */
    public static CubicCurve2D.Double createAdjacentCurve(CubicCurve2D.Double c, double shift1, double shift2) {
        Line2D.Double line1 = new Line2D.Double(c.getX1(), c.getY1(), c.getCtrlX1(), c.getCtrlY1());
        Line2D.Double line2 = new Line2D.Double(c.getX2(), c.getY2(), c.getCtrlX2(), c.getCtrlY2());
        line1 = createParallelLine(line1, shift1);
        line2 = createParallelLine(line2, -shift2);
        return new CubicCurve2D.Double(line1.x1, line1.y1, line1.x2, line1.y2, line2.x2, line2.y2, line2.x1, line2.y1);
    }
            ...
            if (conf.contains(directions[i])) {
                ...
                for (int j = i + 2; j < maxJ; j++) {
                    ...
                    if (conf.contains(directions[j])) {
                        ...
                        if (doubleTrack) {
                            sections.add(createAdjacentCurve(toCurve, doubleTrackGap, doubleTrackGap));
                            sections.add(createAdjacentCurve(toCurve, -doubleTrackGap, -doubleTrackGap));
                        } else {
    ...
    void paintTrack(Graphics2D g, Iterable<Double> sections) {
        ...
        for (CubicCurve2D.Double section : sections) {
            ...
            CubicCurve2D.Double rail1 = createAdjacentCurve(section, halfGauge, halfGauge);
            CubicCurve2D.Double rail2 = createAdjacentCurve(section, -halfGauge, -halfGauge);
            g.draw(rail1);
            g.draw(rail2);
        }
    ...
    void setIcon(String typeName) {
        try {
            ...
            String relativeFileName = "icons" + File.separator + typeName + ".png";
