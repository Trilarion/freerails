...
class ProcessCargoAtStationMoveGenerator {
    ...
    public static ArrayList<Move> processCargo(ReadOnlyWorld w, CargoBatchBundle bundle, int stationID, FreerailsPrincipal p, int trainId) {
        ...
        while (batches.hasNext()) {
            ...
            double distanceSquared = (batch.getSourceP().x - thisStation.p.x) * (batch.getSourceP().x - thisStation.p.x) + (batch.getSourceP().y - thisStation.p.y) * (batch.getSourceP().y - thisStation.p.y);
