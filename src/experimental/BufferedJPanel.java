...
/**
 * I thought this class might improve the frame rate but it seems to make things
 * worse on my machine.
 * 
 * @author Luke
 * 
...
public class BufferedJPanel extends JPanel {
	...
	private static final Logger logger = Logger.getLogger(BufferedJPanel.class
			.getName());
	...
	protected void paintComponent(Graphics g) {
		...
		long repaintsRequested = RepaintManagerForActiveRendering
		...
		if (paints == 200) {
			long percentageUsage = (bufferUsed * 100)
					/ (bufferRepaints + bufferUsed);
			bufferRepaints = 0;
			bufferRepaints = 0;
			paints = 0;
	...
	private void resetBuffer() {
		...
		buffer = defaultConfiguration.createCompatibleVolatileImage(
