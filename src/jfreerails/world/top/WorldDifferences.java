 ...
 * world object are as follows.
 * <p>
 * (1) Uses less memory.
 * </P>
 * <p>
 * (2) Lets you pinpoint where differences on the map are, so you don't need to
 * check every tile.
 * </P>
 * 
 * 
...
public class WorldDifferences implements World {
	...
	private enum KeyType {
		LIST, PLAYER, BANK_ACCOUNT, LIST_LENGTH
	};

	private GameTime time;

	/**
	 * Instances of this class are used as the keys in the hashmap
	 * listDifferences.
	 */
	public class DiffKey {
		...
		/**
		 * Parameters (if present) should be in the following order keyType,
		 * ListKeyNumber, Player, index
		 */
		public DiffKey(KeyType keyType, int a, int b, int c) {
		...
		public DiffKey(KeyType keyType, int a, int b) {
			this(keyType, a, b, -1);
		}

		public DiffKey(KeyType keyType, int a) {
			this(keyType, a, -2, -3);
		}
	...
	/**
	 * Stores the differences not on the map, instances of DiffKey are used as
	 * keys.
	 */
	...
	public void set(ITEM item, FreerailsSerializable element) {
		if (equal(element, underlyingWorld.get(item))) {
			// Case 1: the item is restored to the same value as the underlying
			// world object.
			if (this.listDifferences.containsKey(item)) {
				this.listDifferences.remove(item);
			}
		} else {
			// Case 2: the item is changed to a value different to the
	...
	public void set(KEY key, int index, FreerailsSerializable element,
		...
		DiffKey diffKey = new DiffKey(KeyType.LIST, key.getKeyID(),
		...
		if (underlyingWorld.boundsContain(key, index, principal)
			...
			// Case 1: the element is restored to the same value as the
			// underlying world object.
			if (this.listDifferences.containsKey(diffKey)) {
				this.listDifferences.remove(diffKey);
			}
		} else {
			// Case 2: the element is changed to a value different to the
			// underlying world object.
	...
	public int add(KEY key, FreerailsSerializable element,
			FreerailsPrincipal principal) {
		int playerNumber = getPlayerID(principal);
		DiffKey lengthKey = new DiffKey(KeyType.LIST_LENGTH, key.getKeyID(),
				playerNumber);
		int index;
		int newLength;
		int underLyingSize;
		...
		DiffKey elementKey = new DiffKey(KeyType.LIST, key.getKeyID(),
				playerNumber, index);
	...
	public int add(SKEY key, FreerailsSerializable element) {
		...
		DiffKey elementKey = new DiffKey(KeyType.LIST, key.getKeyID(), index);
		assert !this.listDifferences.containsKey(elementKey);

		if (underLyingSize > index) {
			// We are restoring an element that is present in the underlying
			// world,
			// so we need to check whether it is the same as the element we are
			// adding.
	...
	public FreerailsSerializable removeLast(KEY key,
		...
		// The specified principle may exist on this object but not the
		// underlying world object!
		if (underlyingWorld.isPlayer(principal)) {
			underLyingSize = underlyingWorld.size(key, principal);
		} else {
		...
		/*
		 * If the element to be reomoved is stored in the list differences
		 * hashmap we need to remove it.
		 */
		DiffKey elementKey = new DiffKey(KeyType.LIST, key.getKeyID(),
				playerNumber, index);
	...
	public int addPlayer(Player player) {
		...
		DiffKey playerKey = new DiffKey(KeyType.PLAYER, playerID);
		listDifferences.put(playerKey, player);

		DiffKey bankAccountKey = new DiffKey(KeyType.BANK_ACCOUNT, playerID);
		...
		for (int i = 0; i < KEY.getNumberOfKeys(); i++) {
			DiffKey listLengthKey = new DiffKey(KeyType.LIST_LENGTH, i,
					playerID);
	...
	public FreerailsSerializable get(KEY key, int index, FreerailsPrincipal p) {
		...
		DiffKey diffKey = new DiffKey(KeyType.LIST, key.getKeyID(), this
				.getPlayerID(p), index);
	...
	public int size(KEY key, FreerailsPrincipal p) {
		DiffKey listSize = new DiffKey(KeyType.LIST_LENGTH, key.getKeyID(),
				this.getPlayerID(p));
	...
	public boolean isPlayer(FreerailsPrincipal p) {
		return -1 < getPlayerID(p);
	}

	/** Returns the player number or -1 if the player is not found. */
	private int getPlayerID(FreerailsPrincipal p) {
	...
	public Player getPlayer(int i) {
		DiffKey diffKey = new DiffKey(KeyType.PLAYER, i);
	...
	private boolean isAccountDiff(FreerailsPrincipal p) {
		DiffKey accountKey = new DiffKey(KeyType.BANK_ACCOUNT, this
				.getPlayerID(p));
	...
	private BankAccount addAccountIfNecessary(FreerailsPrincipal p) {
		...
		for (int i = 0; i < underlyingWorld.getNumberOfTransactions(p); i++) {
	 ...
	 * is null.
	 */
	private static boolean equal(FreerailsSerializable a,
			FreerailsSerializable b) {
		if (null == a || null == b) {
	...
	public int addActiveEntity(AKEY key, Activity element,
			FreerailsPrincipal principal) {
	...
	public void add(AKEY key, int index, Activity element,
			FreerailsPrincipal principal) {
		// TODO Auto-generated method stub
		throw new UnsupportedOperationException();
	}

	public Activity removeLastActiveEntity(AKEY key,
			FreerailsPrincipal principal) {
		// TODO Auto-generated method stub
		throw new UnsupportedOperationException();
	}

	public Activity removeLastActivity(AKEY key, int index,
			FreerailsPrincipal principal) {
		// TODO Auto-generated method stub
		throw new UnsupportedOperationException();
	}

	public ActivityIterator getActivities(AKEY key, int index,
