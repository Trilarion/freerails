...
public class SpeedTimeAndStatus  implements FreerailsSerializable{
	...
	public enum Activity{STOPPED_AT_STATION, READY, WAITING_FOR_FULL_LOAD, STOPPED_AT_SIGNAL, CRASHED, NEEDS_UPDATING};
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof SpeedTimeAndStatus)) return false;

        final SpeedTimeAndStatus speedTimeAndStatus = (SpeedTimeAndStatus) o;

        if (speed != speedTimeAndStatus.speed) return false;
        if (!activity.equals(speedTimeAndStatus.activity)) return false;
        if (!t.equals(speedTimeAndStatus.t)) return false;
