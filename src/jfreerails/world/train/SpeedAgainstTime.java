...
public class SpeedAgainstTime implements FreerailsSerializable {
	...
	public static final SpeedAgainstTime STOPPED = new SpeedAgainstTime(
			new GameTime[] { GameTime.BIG_BANG, GameTime.END_OF_THE_WORLD },
			new int[] { 0, 0 });
	...
	public boolean equals(Object o) {
		...
		if (!Arrays.equals(speeds, speedAgainstTime.speeds))
			return false;
		if (!Arrays.equals(times, speedAgainstTime.times))
			return false;
	...
	public int getDistance(GameTime t) {
		...
		long time = t.getTime();
		for (int i = 1; i < times.length; i++) {
			long start = times[i - 1].getTime();
			long end = times[i].getTime();
