...
public class TrainMotionTest extends TestCase {
	...
	public void testConstructor() {
		Step[] vectors = { EAST, EAST, EAST };
		PathOnTiles path = new PathOnTiles(new Point(0, 0), vectors);
		int trainLength = TrainModel.WAGON_LENGTH;
		assertFalse(throwsException(path, 1, trainLength,
				SpeedAgainstTime.STOPPED));
		assertTrue(throwsException(path, 0, trainLength,
				SpeedAgainstTime.STOPPED));
		assertTrue(throwsException(path, 4, trainLength,
				SpeedAgainstTime.STOPPED));
	...
	public void testGetPosition() {
		...
		PathOnTiles path = new PathOnTiles(new Point(0, 0), vectors);
		int trainLength = TrainModel.WAGON_LENGTH;
		TrainMotion motion = new TrainMotion(path, 1, trainLength,
				SpeedAgainstTime.STOPPED);
	...
	public void testNext() {
		...
		TrainMotion motion2 = motion.next(newSpeeds, Step.EAST);
	...
	boolean throwsException(PathOnTiles path, int enginePosition,
			int trainLength, SpeedAgainstTime speeds) {
		try {
