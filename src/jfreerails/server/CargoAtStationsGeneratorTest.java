...
public class CargoAtStationsGeneratorTest extends TestCase {
    ...
    /** If, say, 14 units get added each year, some month we should add 1
     * and others we should add 2 such that over the year exactly 14 units
     * get added.*/
    private void assertCorrectTotalAddedOverYear(final int unitPerYear) {
        ...
        for (int i = 0; i < 12; i++) {
            amountAddedThisSoFar += cargoGenerator.calculateAmountToAdd(unitPerYear,
                i);
        }
