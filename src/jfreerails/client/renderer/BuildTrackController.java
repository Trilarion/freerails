...
public class BuildTrackController implements GameModel {
	...
	public MoveStatus changeTrack(Point a, Point b,
		...
		int mode = trackBuilder.getTrackBuilderMode();
		assert (mode == TrackMoveProducer.REMOVE_TRACK || mode == TrackMoveProducer.UPGRADE_TRACK);
	...
	/**
	 * Attempts to building track from the specifed point in the specified
	...
	public void setProposedTrack(Point startPoint, Point endPoint,
			TrackMoveProducer trackBuilder) {
		assert (trackBuilder.getTrackBuilderMode() != TrackMoveProducer.IGNORE_TRACK);	
		m_buildNewTrack = trackBuilder.getTrackBuilderMode() == TrackMoveProducer.BUILD_TRACK;
	...
	private void updateSearch() {
		...
		if (searchStatus() == IncrementalPathFinder.PATH_FOUND) {
			if (m_buildNewTrack) {
				...
				Integer mode = (Integer) m_modelRoot
				...
				for (OneTileMoveVector v : m_path) {
					Move move;
					attmeptMove: {
						if (TrackMoveProducer.REMOVE_TRACK == mode.intValue()) {
							try {
							...
							assert TrackMoveProducer.UPGRADE_TRACK == mode
									.intValue();
