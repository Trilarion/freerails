...
public class IncomeStatementGeneratorTest extends TestCase {
	...
	private void addTrans(String category, Money amount) {
		for(int i = 0; i < w.size(SKEY.CARGO_TYPES); i++){
			...
			if(ct.getCategory().equals(category)){
				...
				w.addTransaction(new DeliverCargoReceipt(amount, 10, 0, cb), MapFixtureFactory.TEST_PRINCIPAL);
				return;
			}
		}		
	...
	protected void setUp() throws Exception {
		...
		balanceSheetGenerator = new IncomeStatementGenerator(w, MapFixtureFactory.TEST_PRINCIPAL);
	}
}
