...
/** Generates the balance sheet - note, its fields are read using reflection so don't change their
 * names.
 * @author Luke
 *
 */
public class BalanceSheetGenerator {
    ...
    BalanceSheetGenerator(ReadOnlyWorld w, FreerailsPrincipal principal) {
        ...
        TransactionAggregator ytdOperatingFunds = new TransactionAggregator(w,
                ...
                protected boolean condition(int i) {
                    int transactionYear = cal.getYear(w.getTransactionTimeStamp(
                                i, principal).getTime());
    ...
    private Money calTrackTotal(int category) {
        ItemsTransactionAggregator aggregator = new ItemsTransactionAggregator(w,
                principal);
        aggregator.setStartYear(startyear);
