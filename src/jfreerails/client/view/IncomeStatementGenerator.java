...
public class IncomeStatementGenerator {
	...
	public Money mailTotal, passengersTotal, fastFreightTotal,
			slowFreightTotal, bulkFreightTotal, interestTotal,
			trainMaintenanceTotal, trackMaintenanceTotal,
			stationMaintenanceTotal, profitTotal;
	
	public Money mailYtd, passengersYtd, fastFreightYtd,
	slowFreightYtd, bulkFreightYtd, interestYtd,
	trainMaintenanceYtd, trackMaintenanceYtd,
	stationMaintenanceYtd, profitYtd;
	...
	Money calTotal(int transactionCategory) {
		long amount = 0;
		for (int i = 0; i < w.getNumberOfTransactions(this.principal); i++) {
			Transaction t = w.getTransaction(i, principal);
			GameTime time = w.getTransactionTimeStamp(i, principal);
			if (t.getCategory() == transactionCategory && cal.getYear(time.getTime()) >= this.startyear) {
