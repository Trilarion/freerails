	...
	public void processMove(Move move) {
		...
		if (move instanceof TransferCargoAtStationMove) {
			...
			if (positiveRevenue && isRightPlayer) {
				...
				int trainCargoBundle = transferCargoAtStationMove
						.getChangeOnTrain().getIndex();
				int stationCargoBundle = transferCargoAtStationMove
						.getChangeAtStation().getIndex();
				...
				for (int i = 0; i < world.size(SKEY.CARGO_TYPES); i++) {
					int amount = transferCargoAtStationMove
							.getQuantityOfCargo(i);

					if (amount > 0) {
						CargoType ct = (CargoType) world.get(SKEY.CARGO_TYPES,
								i);
				...
				if (!transferCargoAtStationMove.isWaitingForFullLoad()) {
					try {
						soundManager.playSound(
								"/jfreerails/client/sounds/whistle.wav", 0);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
