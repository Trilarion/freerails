...
public class MarkAsCrashSiteMove extends ChangeItemInListMove{
    ...
    private MarkAsCrashSiteMove(int index,
        FreerailsSerializable before, FreerailsSerializable after,
        FreerailsPrincipal p) {
        super(KEY.TRAIN_POSITIONS, index, before, after, p);
    }
    public static MarkAsCrashSiteMove generateMove(int index, ReadOnlyWorld w, FreerailsPrincipal p) {
        TrainPositionOnMap pos = (TrainPositionOnMap)w.get(KEY.TRAIN_POSITIONS, index, p);
        TrainPositionOnMap newPos = TrainPositionOnMap.createInstance(pos.getXPoints(), pos.getYPoints());
