...
public class RemoveTrainPositionOnMapMove extends RemoveItemFromListMove{
	...
	private RemoveTrainPositionOnMapMove(int index, FreerailsSerializable position, FreerailsPrincipal p) {
      super(KEY.TRAIN_POSITIONS, index, position, p);
   }
    
    public static Move generateMove(int index, FreerailsPrincipal p, ReadOnlyWorld world) {
        
        FreerailsSerializable position = (TrainPositionOnMap)world.get(KEY.TRAIN_POSITIONS, index, p);
